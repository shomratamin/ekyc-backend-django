...
if (gFlow === undefined) var gFlow = {};

gFlow.defaultComponent = "_base";
gFlow.preservePropertySections = true;
gFlow.dragIcon = 'icon';//icon = use component icon when dragging | html = use component html to create draggable element

gFlow.baseUrl =  document.currentScript?document.currentScript.src.replace(/[^\/]*?\.js$/,''):'';

gFlow.ComponentsGroup = {};
gFlow.BlocksGroup = {};
...
*/
	...
	render: function(type) {
		...
		if (!(gFlow.preservePropertySections && section.length))
		{
			componentsPanelSections[defaultSection].html('').append(tmpl("gflow-input-sectioninput", {key:"default", header:component.name}));
		...
		if (component.beforeInit) component.beforeInit(gFlow.Builder.selectedEl.get(0));
		...
		var fn = function(component, property) {
			return property.input.on('propertyChange', function (event, value, input) {
					...
					var element = gFlow.Builder.selectedEl;
					...
					if (property.htmlAttr)
					{
						...
						{
							element = gFlow.StyleManager.setStyle(element, property.key, value);
						}
						else if (property.htmlAttr == "innerHTML") 
						{
							element = gFlow.ContentManager.setHtml(element, value);
						}
						else
						{
						...
						gFlow.Undo.addMutation({type: 'attributes', 
					...
					if (!property.child && !property.parent) gFlow.Builder.selectNode(element);
					
					return element;
			});				
		};			
	
		var nodeElement = gFlow.Builder.selectedEl;
		...
		for (var i in component.properties)
		{
			...
			} else if (property.htmlAttr)
			{
				if (property.htmlAttr == "style")
				{
					...
					var value = gFlow.StyleManager.getStyle(element, property.key);//getStyle returns declared style
				} else
				if (property.htmlAttr == "innerHTML")
				{
					var value = gFlow.ContentManager.getHtml(element);
				} else
				{
			...
			{
				var row = $(tmpl('gflow-property', property)); 
		...
		if (component.init) component.init(gFlow.Builder.selectedEl.get(0));
	}
};	
...
gFlow.WysiwygEditor = {
	...
	destroy: function(element) {
		...
		gFlow.Undo.addMutation({type:'characterData', 
								target: node, 
...
/* controls */    	
	loadControlGroups : function() {	
		...
		componentsList.each(function ()
		{
			...
			for (group in gFlow.ComponentsGroup)	
			{
				...
				components = gFlow.ComponentsGroup[ group ];
				
				for (i in components)
				{
					componentType = components[i];
					component = gFlow.Components.get(componentType);
	...
	loadBlockGroups : function() {	
		...
		blocksList.each(function ()
		{
			...
			for (group in gFlow.BlocksGroup)	
			{
				...
				blocks = gFlow.BlocksGroup[ group ];

				for (i in blocks)
				{
					blockType = blocks[i];
					block = gFlow.Blocks.get(blockType);
	...
	loadUrl : function(url, callback) {	
		...
		jQuery.get(url,function(data){
			gFlow.CodeEditor.codemirror.setValue(data)
		})
		if (gFlow.Builder.iframe.src != url){ 
			// gFlow.Builder.iframe.src = url;
...
/* iframe */
	_loadIframe : function(url) {	
	    ...
	    return this.documentFrame.on("load", function() 
        {
				...
				gFlow.WysiwygEditor.init(window.FrameDocument);
    ...
    _frameLoaded : function() {
		...
		// self.frameHead.append('<link data-gflow-helpers href="' + gFlow.baseUrl + '../../css/gflowjs-editor-helpers.css" rel="stylesheet">');

		self._initHighlight();
		
		$(window).triggerHandler("gflow.iframe.loaded", self.frameDoc);
    },	
	...
	loadNodeComponent:  function(node) {
		data = gFlow.Components.matchNode(node);
		var component;
		
		if (data) 
			component = data.type;
		else 
			component = gFlow.defaultComponent;
			
		gFlow.Components.render(component);
	...
	selectNode:  function(node) {
		...
		{
			gFlow.WysiwygEditor.destroy(self.texteditEl);
...
/* iframe highlight */    
    _initHighlight: function() {
		...
		self.frameHtml.on("mouseup touchend", function(event) {
			if (self.isDragging)
			{
				...
				if (self.dragMoveMutation === false)
				{
					gFlow.Undo.addMutation({type: 'childList', 
				...
				} else
				{
					...
					gFlow.Undo.addMutation(self.dragMoveMutation);
		...
		self.frameHtml.on("dblclick", function(event) {
			...
			if (gFlow.Builder.isPreview == false)
			{
				self.texteditEl = target = jQuery(event.target);

				gFlow.WysiwygEditor.edit(self.texteditEl);
	...
	_initBox: function() {
		...
		$("#down-btn").on("click", function(event) {
			...
			gFlow.Undo.addMutation({type: 'move', 
									target: node,
									oldParent: oldParent,
									newParent: newParent,
		...
		$(".components-list li ol li", addSectionBox).on("click", function(event) {
			var html = gFlow.Components.get(this.dataset.type).html;
		...
		$(".blocks-list li ol li", addSectionBox).on("click", function(event) {
			var html = gFlow.Blocks.get(this.dataset.type).html;
...
/* drag and drop */
	_initDragdrop : function() {
		...
		$('.drag-elements-sidepane ul > li > ol > li').on("mousedown touchstart", function(event) {
			...
			if ($this.data("drag-type") == "component")
				self.component = gFlow.Components.get($this.data("type"));
			else
				self.component = gFlow.Blocks.get($this.data("type"));
	...
	removeHelpers: function (html, keepHelperAttributes = false)
	{
		...
		html = html.replace(/<.*?data-gflow-helpers.*?>/gi, "");
		//attributes
		if (!keepHelperAttributes)
		{
			html = html.replace(/\s*data-gflow-\w+(=["'].*?["'])?\s*/gi, "");
		}
	...
	getHtml: function(keepHelperAttributes = true) 
	{
		...
		$(window).triggerHandler("gflow.getHtml.before", doc);
		...
		"<!DOCTYPE "
         ...
         var filter = $(window).triggerHandler("gflow.getHtml.after", html);
         if (filter) return filter;
         return html;
	},
	getCode: function(){
		return gFlow.CodeEditor.codemirror.getValue()
	},
	...
	saveAjax: function(name, content, callback)
	{
		var data = {};
		data["name"] = name
		...
		$.ajax({
			type: "POST",
			url: 'update-page/',//set your server side save script url
			data: data,
			cache: false,
...
gFlow.CodeEditor = {
	...
	init: function(doc) {
		$("#gflow-code-editor textarea").val(gFlow.Builder.getHtml());

		$("#gflow-code-editor textarea").keyup(function () 
		{
			// delay(gFlow.Builder.setHtml(this.value), 1000);
		});

		//load code on document changes
		gFlow.Builder.frameBody.on("gflow.undo.add gflow.undo.restore", function (e) { gFlow.CodeEditor.setValue();});
	...
	getCode: function(){
		let _code = $("#gflow-code-editor textarea").val();
		return _code;
	},
...
gFlow.Gui = {
	...
	init: function() {
		$("[data-gflow-action]").each(function () {
			on = "click";
			if (this.dataset.gflowOn) on = this.dataset.gflowOn;
			
			$(this).on(on, gFlow.Gui[this.dataset.gflowAction]);
			if (this.dataset.gflowShortcut)
			{
				$(document).bind('keydown', this.dataset.gflowShortcut, gFlow.Gui[this.dataset.gflowAction]);
				$(window.FrameDocument, window.FrameWindow).bind('keydown', this.dataset.gflowShortcut, gFlow.Gui[this.dataset.gflowAction]);
			}
		});
	},
	
	undo : function () {
		if (gFlow.WysiwygEditor.isActive) 
		{
			gFlow.WysiwygEditor.undo();
		} else
		{
			gFlow.Undo.undo();
		}
		gFlow.Builder.selectNode();
	},
	
	redo : function () {
		if (gFlow.WysiwygEditor.isActive) 
		{
			gFlow.WysiwygEditor.redo();
		} else
		{
			gFlow.Undo.redo();
		}
	...
	save : function () {
		$('#textarea-modal textarea').val(gFlow.Builder.getHtml());
		$('#textarea-modal').modal();
	},
	
	//post html content through ajax to save to filesystem/db
	saveAjax : function () {
		var name = gFlow.FileManager.getCurrentName();
		var content = gFlow.Builder.getCode()
		// var content = gFlow.CodeEditor.getCode();
		
		return gFlow.Builder.saveAjax(name, content, function (data) {
			// $('#message-modal').modal().find(".modal-body").html("File saved at: " + data);
		});		
	},

	delete_current_page: function () {
		var name = gFlow.FileManager.getCurrentName();
		var reply = confirm("Are you sure you want to delete this page?");
		if (reply == true) {
			console.log('delete the page')
		} else {
			console.log('cancel action')
		}
	},

	format_selected_code: function () {
		// gFlow.CodeEditor.codemirror.format_selection()
	},
	
	download : function () {
		filename = /[^\/]+$/.exec(gFlow.Builder.iframe.src)[0];
		uriContent = "data:application/octet-stream,"  + encodeURIComponent(gFlow.Builder.getHtml());
	...
	toggleEditor : function () {
		$("#gflow-builder").toggleClass("bottom-panel-expand");
		$("#toggleEditorJsExecute").toggle();
		gFlow.CodeEditor.toggle();
	},
	
	toggleEditorJsExecute : function () {
		gFlow.Builder.runJsOnSetHtml = this.checked;
	},
	
	preview : function () {
		(gFlow.Builder.isPreview == true)?gFlow.Builder.isPreview = false:gFlow.Builder.isPreview = true;
		$("#iframe-layer").toggle();
		$("#gflow-builder").toggleClass("preview");
	},
...
//Pages, file/components tree 
	newPage : function () {
		...
		newPageModal.modal("show").find("form").off("submit").submit(function( event ) {
			...
			var url = startTemplateUrl + '&name=' + name;
			

			var data = {}
			data['url'] = url;
			data['title'] = title;
			gFlow.FileManager.addPage(name, data);
			event.preventDefault();

			gFlow.FileManager.loadPage(name);
			gFlow.FileManager.scrollBottom();
	...
	setDesignerMode : function () {
		...
		gFlow.Builder.setDesignerMode(designerMode);
	},
//layout
	...
	toggleFileManager: function () {
		gFlow.Gui.togglePanel("#filemanager", "--builder-filemanager-height");
	},
	
	toggleLeftColumn: function () {
		gFlow.Gui.togglePanel("#left-panel", "--builder-left-panel-width");
	},
	
	toggleRightColumn: function () {
		gFlow.Gui.togglePanel("#right-panel", "--builder-right-panel-width");
		var rightColumnEnabled = this.attributes["aria-pressed"].value == "true";

		$("#gflow-builder").toggleClass("no-right-panel");
		$(".component-properties-tab").toggle();
		
		gFlow.Components.componentPropertiesElement = (rightColumnEnabled ? "#right-panel" :"#left-panel") +" .component-properties";
...
gFlow.FileManager = {
	...
	init: function() {
		...
		$(this.tree).on("click", "li[data-page] label", function (e) {
			...
			if (page) gFlow.FileManager.loadPage(page);
			return false;			
		})
		...
		$(this.tree).on("click", "li[data-component] label ", function (e) {
			...
			gFlow.Builder.frameHtml.animate({
				scrollTop: $(node).offset().top
			}, 1000);

			gFlow.Builder.selectNode(node);
			gFlow.Builder.loadNodeComponent(node);
	...
	addPage: function(name, data) {
		...
		if (data.folder)
		{
			if (!(folder = this.tree.find('li[data-folder="' + data.folder + '"]')).length) 
			{
				...
				folder = $(tmpl("gflow-filemanager-folder", data));
		...
		folder.append(
			tmpl("gflow-filemanager-page", data));
	},
	...
	addComponent: function(name, url, title, page) {
		$("[data-page='" + page + "'] > ol", this.tree).append(
			tmpl("gflow-filemanager-component", {name:name, url:url, title:title}));
	},
				...
				if (node.hasChildNodes()) {
					for (var j = 0; j < node.childNodes.length; j++) {
						...
						if (child && child["attributes"] != undefined && 
							(matchChild = gFlow.Components.matchNode(child))) 
						{
		...
		function drawComponentsTree(tree)
		{
			...
			for (i in tree)
			{
				...
				if (tree[i].children.length > 0) 
				{
					...
					<label for="id' + j + '" style="background-image:url(' + node.image + ')"><span>' + node.name + '</span></label>\
	...
	loadPage: function(name, allowedComponents = false, disableCache = true) {
		...
		gFlow.Builder.loadUrl(url + (disableCache ? (url.indexOf('?') > -1?'&':'?') + Math.random():''), 
			function () { 
				gFlow.FileManager.loadComponents(allowedComponents); 
			});
	},
